    ...
    def recreate_img_filled(frame, im_grids, points_grids, list_transform_matrix):
        ...
        for im_grid, points_grid, transform_matrix in zip(im_grids, points_grids, list_transform_matrix):
            if im_grid is None:
                ...
                #     x, y = point
                #     try:
                #         cv2.circle(new_im, (int(x), int(y)), 6, (255, 0, 0), 3)
        ...
        red_overlay = np.zeros_like(frame)  # Initialize an image with the same size as the frame
        red_overlay[:] = (99, 60, 22)  # Set all pixels to red (BGR format)

        # Mask the red overlay to match the non-black regions of the original mask
        red_masked = cv2.bitwise_and(red_overlay, red_overlay, mask=mask)
        frame[mask > 0] = red_masked[mask > 0]
        im_final = cv2.bitwise_and(frame, frame)
